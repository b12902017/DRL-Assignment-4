...
agent = SAC(state_dim, action_dim, hidden_dim=64, device=device)
memory = MemoryBuffer(100000, device=device)
episode = 1000 # can try 1000
init_explor = 10000
batch_size = 64
__eval = True
...
for epi in range(episode):
    ...
    if (epi+1) % 50 == 0 and __eval is True:
        avg_reward = 0.
        episodes = 10
        ...
        if (epi+1) % 100 == 0 and avg_reward > -200:
            torch.save({'policy_state_dict': agent.policy.state_dict(),
                    'critic_state_dict': agent.critic.state_dict()}, f"Q1_episode{epi+1}.pt")
